   ...
   */
   public class FrameBuffer {
    ...
    private String getClientIP() {
      ...
      try {
        clientIP = ((InetSocketAddress)((TNonblockingSocket)(this.trans_))
                .getSocketChannel().getRemoteAddress()).getAddress().getHostAddress();
      } catch (Throwable t) {
      }
    ...
    private int getClientPort() {
      ...
      try {
        port = ((InetSocketAddress)(
                ((TNonblockingSocket)(this.trans_)).getSocketChannel().getRemoteAddress())).getPort();
      } catch (Throwable t) {
      }
    ...
    public boolean read() {
      if (state_ == FrameBufferState.READING_FRAME_SIZE) {
        ...
        if (buffer_.remaining() == 0) {
          ...
          if (frameSize <= 0) {
            LOGGER.error("Client[" + getClientIP() + ":" + getClientPort() + "] Read an invalid frame size of " + frameSize
          ...
          if (frameSize > MAX_READ_BUFFER_BYTES) {
            LOGGER.error("Client[" + getClientIP() + ":" + getClientPort() + "] Read a frame size of " + frameSize
    ...
    public void invoke() {
      ...
      try {
        ...
        // 将客户端的IP和端口事先保存起来
        ((TBinaryProtocol)inProt_).clientIP = getClientIP();
        ((TBinaryProtocol)inProt_).port = getClientPort();
     ...
     *         connection closed.
     *         sdk上报日志的线程强行关闭导致，不是logserver的问题 -- Haiyang
     */
    private boolean internalRead() {
      try {
        ...
        LOGGER.warn("client {}:{} Got an IOException in internalRead! | becasue client was not close Transport",
